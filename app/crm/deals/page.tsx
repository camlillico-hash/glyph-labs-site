...
const STAGES = ["Warm intro booked", "Warm intro completed", "90-min disco booked", "90-min disco completed", "Proposal / commitment", "Launch paid (won)", "Lost"];
...
export default function DealsPage() {
  ...
  const visibleDeals = useMemo(() => deals.filter((d) => !["Launch paid (won)", "Lost"].includes(d.stage)), [deals]);
  ...
  const stageWeight = (stage: string) => stage === "Warm intro booked" ? 10 : stage === "Warm intro completed" ? 15 : stage === "90-min disco booked" ? 25 : stage === "90-min disco completed" ? 35 : stage === "Proposal / commitment" ? 50 : stage === "Launch paid (won)" ? 100 : 0;
          ...
          {rows.map((d) => {
            ...
            return (
              <tr key={d.id} className="border-b border-neutral-900 hover:bg-neutral-900/60" onDoubleClick={() => startInlineEdit(d)}>
                ...
                <td className="px-3 py-2 text-slate-300" onClick={() => !editing && startInlineEdit(d)}>{editing ? <select className="crm-input" value={inlineDraft.contactId || ""} onChange={(e)=>setInlineDraft({...inlineDraft, contactId:e.target.value})}><option value="">Select linked person *</option>{contacts.map((c) => <option key={c.id} value={c.id}>{c.firstName} {c.lastName}</option>)}</select> : (d.contactId ? <a className="text-sky-300 hover:text-sky-200" onClick={(e)=>e.stopPropagation()} href={`/crm/${(contacts.find((c) => c.id === d.contactId)?.pipelineType || 'connector') === 'connector' ? 'connectors' : 'leads'}?contactId=${d.contactId}`}>{contactName(d.contactId)}</a> : "—")}</td>
        ...
        <div className="overflow-x-auto pb-2"><div className="flex gap-4 min-w-max">
          {STAGES.map((stage) => (
            <div key={stage} className={`crm-card p-3 w-[240px] shrink-0 transition-all duration-150 ${hoverStage === stage ? "ring-2 ring-emerald-500/80 border-emerald-500/70" : ""}`} onDragOver={(e) => e.preventDefault()} onDragEnter={() => setHoverStage(stage)} onDragLeave={() => setHoverStage((s) => s === stage ? null : s)} onDrop={async () => { if (!draggingDealId) return; await moveDealStage(draggingDealId, stage); setDraggingDealId(null); setHoverStage(null); setHoverDrop(null); }}>
              <h3
                className="mb-3 inline-flex items-center gap-1.5 font-semibold text-emerald-300"
                      ...
                      {stageDeals.map((d, idx) => (
                        <div key={d.id}>
                          ...
                          <button draggable onDragStart={() => setDraggingDealId(d.id)} onDragEnd={() => { setDraggingDealId(null); setHoverStage(null); setHoverDrop(null); }} className={`crm-card bg-neutral-950 w-full min-w-0 p-2 text-left cursor-grab transition-all duration-150 ${draggingDealId === d.id ? "scale-[1.02] opacity-70" : ""}`} onClick={() => openTray(d)}>
                            ...
                            <p className="truncate text-xs text-slate-500">{d.contactId ? <a className="text-sky-300 hover:text-sky-200" onClick={(e)=>e.stopPropagation()} href={`/crm/${(contacts.find((c) => c.id === d.contactId)?.pipelineType || 'connector') === 'connector' ? 'connectors' : 'leads'}?contactId=${d.contactId}`}>{contactName(d.contactId)}</a> : "—"}</p>
                      ...
                      {dealStamps.map((s) => {
                        ...
                        return (
                          <tr key={s.id} className="border-b border-neutral-900 hover:bg-neutral-900/60">
                            ...
                            <td className="px-3 py-2 text-slate-300">{linkedDeal?.contactId ? <a className="text-sky-300 hover:text-sky-200" href={`/crm/${(contacts.find((c) => c.id === linkedDeal?.contactId)?.pipelineType || 'connector') === 'connector' ? 'connectors' : 'leads'}?contactId=${linkedDeal.contactId}`}>{contactName(linkedDeal.contactId)}</a> : "—"}</td>
        ...
        <div className="fixed inset-0 z-40">
          ...
          <aside className="absolute right-0 top-0 flex h-full w-full max-w-xl flex-col border-l border-neutral-700 bg-neutral-950 p-5 shadow-2xl">
            ...
            >
              ...
              <Field label="Linked person" editMode={editMode || createMode} read={!(editMode || createMode) ? (draft.contactId ? <a className="text-sky-300 hover:text-sky-200" href={`/crm/${(contacts.find((c) => c.id === draft.contactId)?.pipelineType || 'connector') === 'connector' ? 'connectors' : 'leads'}?contactId=${draft.contactId}`}>{contactName(draft.contactId)}</a> : "—") : undefined}><select className="crm-input" value={draft.contactId || ""} onChange={(e) => setDraft({ ...draft, contactId: e.target.value })}><option value="">Select linked person *</option>{contacts.map((c) => <option key={c.id} value={c.id}>{c.firstName} {c.lastName} {c.email ? `(${c.email})` : ""}</option>)}</select></Field>
